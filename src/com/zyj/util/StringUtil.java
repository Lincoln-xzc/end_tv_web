...
public class StringUtil {
	...
	private static int task_no = 100001;
	/**
	 * 获取唯一数，订单号，11开头
	 */
	public static synchronized String  getDateRandowOrderNo(){
		SimpleDateFormat tempDate = new SimpleDateFormat("yyMMdd" + "" + "HHmmss");
		String datetime = tempDate.format(new java.util.Date());    //12位
		...
		datetime = "11"+ datetime + randomInt+"" + task_no;
	 ...
	 * 获取唯一数，支付单号，22开头
	 */
	public static synchronized String  getDateRandowPayNo(){
		SimpleDateFormat tempDate = new SimpleDateFormat("yyMMdd" + "" + "HHmmss");
		String datetime = tempDate.format(new java.util.Date());    //12位
		...
		datetime =  "22"+datetime + randomInt+"" + task_no;
...
//		System.out.println(getNextNo("A","A20140919-043"));
		System.out.println(getDateRandowOrderNo());
		System.out.println(getDateRandow());
	}
	...
	public static String formatImageUrl(Object obj){
		...
		try{
			if(isNotEmpty(obj)){
				url = FileUtil.getFileFullUrl(ToString(obj));
			}
		}catch(Exception e){
			e.printStackTrace();
		}
	 ...
	 * @return
	 */
	public static List<Object> formatObjList(Object obj, String valueType){
		List<Object> list = new ArrayList<>();
		if(obj==null)
			return list;
		if(valueType==null)
			valueType="";
		String[] objs = ToString(obj).split(",");
		for(String os : objs){
			if(StringUtils.isNotEmpty(os)){
				switch(valueType){
					case "Long" :list.add(NumberUtil.parseLong(os));break;
					case "Integer" : list.add(NumberUtil.parseInt(os)); break;
					default : list.add(os);
				}
	 ...
	 * @return
	 */
	public static List<String> formatFileUrlList(Object obj){
		List<String> fileList = new ArrayList<>();
		if(obj==null)
			return fileList;
		
		String[] urls = ToString(obj).split(",");
		for(String url : urls){
			if(StringUtils.isNotEmpty(url))
				fileList.add(FileUtil.getFileFullUrl(url));
		}
		return fileList;
	}
	 ...
	 * 把大图url转换为小图url
	 */
	public static List<String> changeImgBigUrlToSmall(List<String> imgList){
		List<String> fileList = new ArrayList<>();
		...
		for(String img : imgList){
			...
			fileList.add(img.substring(0,length)+"_s"+img.substring(length));
		}
		return fileList;
	}
